...
<Note>
  **API keys are issued to Enterprise accounts.** On every other plan, use the [MCP server](/mcp-server) — it authenticates as you over OAuth, needs no key, and exposes the same research engine.
</Note>
